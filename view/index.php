...
if (isset($_GET['sessiontest'])) {
    if (!isset($_SESSION['test'])) {
        $_SESSION['test'] = 1;
        echo "Session test set. Reload the page.";
    } else {
        echo "Session test value: " . $_SESSION['test'];
    }
    exit;
}
// DEBUG: Show session variables for troubleshooting
if (isset($_GET['debug'])) {
  ...
  <body class="gradient-bg min-h-screen">
    ...
    class="fixed inset-0 z-50  flex items-center justify-center hidden">
      ...
      style="max-height: 85vh">
        ...
        <form action="<?php echo htmlspecialchars('../controller/signin.php', ENT_QUOTES, 'UTF-8') ?>" method="post" id="signupForm">
          ...
          <form action="<?php echo htmlspecialchars('../controller/login.php', ENT_QUOTES, 'UTF-8') ?>" method="post">
              <div class="mb-4">
                <label
                  for="loginEmail"
                class="block text-sm font-medium text-gray-300 mb-1.5">Email</label>
          ...
          <div id="forgotPasswordStep" class="form-section hidden flex flex-col justify-center min-h-[400px] p-8">
              <div class="mb-6 text-center">
                ...
                <?php else: ?>
                  ...
                  <form action="<?php echo htmlspecialchars('../controller/forgot.php', ENT_QUOTES, 'UTF-8') ?>" method="post" class="flex flex-col gap-4">
                    <div>
                    <label
                for="email"
                class="block text-sm font-medium text-gray-300 mb-1.5">Email</label>
        ...
        class="relative h-[600px] mb-24 container mx-auto flex justify-center items-center hidden md:flex">
          ...
          style="transform: translateY(200px)">
            ...
            class="p-0 h-full transform flex items-center justify-center bg-[#0A0A0B]">
              <img
                src="../assets/images/WhatsApp Image 2025-04-26 at 02.54.18.jpeg"
    ...
    <script src="../assets/js/script.js?v=forcecache"></script>
...
<script>
// Remove ?showForgot=1 from the URL without reloading the page
if (window.location.search.includes('showForgot=1')) {
  const url = new URL(window.location);
  url.searchParams.delete('showForgot');
...
<script>
  // star button
window.addEventListener('DOMContentLoaded', function() {
  var tryItFreeBtn = document.querySelector('.star-btn');
  if (tryItFreeBtn) {
    tryItFreeBtn.addEventListener('click', function(e) {
      e.preventDefault();
      var loginModal = document.getElementById('login');
      if (loginModal) {
        loginModal.classList.remove('hidden');
        document.body.style.overflow = 'hidden';
        // Hide all sections, show only login
        var forgotStep = document.getElementById('forgotPasswordStep');
        if (loginForm) loginForm.classList.remove('hidden');
      }
    });
